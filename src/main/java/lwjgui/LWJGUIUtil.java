...
public class LWJGUIUtil {
	...
	private static long createOpenGLWindow(String name, int width, int height, boolean resizable, boolean ontop,
			boolean modernOpenGL) {
		// Configure GLFW
	...
	/**
	 * Creates a standard OpenGL 3.2 window with a Core Profile. On Windows OS there
	 * is not a difference between old and new OpenGL contexts, graphics drivers
	 * allow for the mixing of these two.<br>
	 * <br>
	 * On Mac OS there is a difference between old and new OpenGL contexts. If you
	 * wish to use modern OpenGL commands, they must be done on a OpenGL 3.2 Core
	 * context. If you wish to use depricated OpenGL commands, they must be done on
	 * a OpenGL 2.0 Non Core context.
	 * 
	 * @param name
	 * @param width
	 * @param height
	 ...
	 * On Windows OS there is not a difference between old and new OpenGL contexts,
	 * graphics drivers allow for the mixing of these two.<br>
	 * <br>
	 * On Mac OS there is a difference between old and new OpenGL contexts. If you
	 * wish to use modern OpenGL commands, they must be done on a OpenGL 3.2 Core
	 * context. If you wish to use depricated OpenGL commands, they must be done on
	 ...
	 * @return GLFW Window Handle
	 */
	public static long createOpenGLDepricatedWindow(String name, int width, int height, boolean resizable,
			boolean ontop) {
		return createOpenGLWindow(name, width, height, resizable, ontop, false);
	}

	/**
	 * Fills a NanoVG rectangle at a given x/y with a given width/height and a
	 * specified color.
	 * 
	 * @param context
	 * @param x
	...
	/**
	 * Fills a rounded NanoVG rectangle at a given x/y with a given width/height and
	 * a specified color.
	 * 
	 * @param context
	 * @param x
	 ...
	 * @param color
	 */
	public static void fillRoundRect(Context context, double x, double y, double width, double height, double radius,
			Color color) {
		if (color == null)
			return;
	...
	/**
	 * Outlines a NanoVG rectangle at a given x/y with a given width/height and a
	 * specified color.
	 * 
	 * @param context
	 * @param x
	...
	/**
	 * Restarts the Java virtual machine and forces it to run on the first thread IF
	 * and only IF it is not currently on the first thread. This allows your LWJGL3
	 * program to run on Mac properly. <br>
	 * Your java program must return after calling this method if it returns true as
	 * to prevent your application from running twice. <br>
	 * <br>
	 * To implement this method, simply put it on the first line of your main(args)
	 * function. <br>
	 * <br>
	 * Credit goes to Spasi on JGO for making this utility. <br>
	 * <br>
	 ...
	 * }<br>
	 * 
	 * @param needsOutput - Whether or not the JVM should print to
	 *                    System.out.println
	...
	public static void drawText(String text, Font font, FontStyle style, double size, Color color, double x, double y,
			Pos alignment, Context vg) {
