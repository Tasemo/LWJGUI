...
public abstract class Node implements Resizable {
	...
	public void updateChildren() {
		for (int i = 0; i < children.size(); i++) {
			Node child = children.get(i);
			if (child != null) {
				child.position(this, cached_context);
			}
		}
	}
	...
	/**
	 * Computes the local position by comparing the absolute position to the parents
	 * absolute position.
	 */
	...
	protected void position(Node parent) {
		...
		cached_context = parent.cached_context;
	...
	protected void position(Node parent, Context c) {
		Node oldParent = this.parent;
		this.parent = parent;
	...
	/**
	 * Calculates the bounding of this node based on its own position and the
	 * positions of its children. Meant to be used for some rendering (E.G. filling
	 * in the background) and input handling (ensuring that every child in the node
	 * is clickable).
	 */
	public void calculateNodeBounds() {
		...
		// System.out.println("calculateNodeBounds() "+ this + " -> " + sx + " " + sy +
		// " " + getWidth() + " " + getHeight());
	...
	protected void clip(Context context, int padding) {
		...
		NanoVG.nvgScissor(context.getNVG(), clipBoundsTemp.minX - padding, clipBoundsTemp.minY - padding,
				clipBoundsTemp.getWidth() + padding * 2, clipBoundsTemp.getHeight() + padding * 2);
	}
	...
	public boolean isDescendentHovered() {
		Node hovered = cached_context.getHovered();
		if (hovered == null) {
			return false;
		}
	...
	/**
	 * Set the preferred size of this node. <br>
	...
	/**
	 * Set the preferred width of this node. <br>
	 * This size is not guaranteed.
	 * 
	 * @param width
	 */
	...
	/**
	 * Set the preferred height of this node. <br>
	 * This size is not guaranteed.
	 * 
	 * @param width
	 */
	...
	protected boolean onMouseReleased(double mouseX, double mouseY, int button) {
		...
		if (mouseReleasedEventInternal != null
				&& EventHelper.fireEvent(mouseReleasedEventInternal, new MouseEvent(mouseX, mouseY, button))) {
			consumed = true;
		}

		if (mouseReleasedEvent != null
				&& EventHelper.fireEvent(mouseReleasedEvent, new MouseEvent(mouseX, mouseY, button))) {
			consumed = true;
		}
	...
	/**
	 * Returns whether or not this node (and all of hits children) will ignore the
	 * mouse.
	 * 
	 * @return
	 */
	...
	/**
	 * Flag that controls whether this node (and all of its children) ignore the
	 * mouse.
	 * 
	 * @param t
	 */
