...
public class ScrollPane extends FillableRegion {
	...
	private void update() {
		...
		if (content != null) {
			...
			content.setAbsolutePosition(internalScrollCanvas.getX() + this.padding.getLeft() - hBar.pixel,
					internalScrollCanvas.getY() + this.padding.getTop() - vBar.pixel);
	...
	private class ScrollBar {
		...
		public Vector4d getBounds() {
			if (this.orientation.equals(Orientation.HORIZONTAL)) {
				return new Vector4d((int) (getX() + pixelSpaceToScrollSpace(pixel)),
						(int) (getY() + viewportSize.y + barPadding), (int) Math.max(length, 1), (int) thickness);
			} else {
				return new Vector4d((int) (getX() + viewportSize.x + barPadding),
						(int) (getY() + pixelSpaceToScrollSpace(pixel)), (int) thickness, (int) Math.max(length, 1));
			}
		}
