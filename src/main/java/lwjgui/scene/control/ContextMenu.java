...
public class ContextMenu extends PopupWindow {
	...
	public void show(Scene scene, double absoluteX, double absoluteY) {
		this.returnNode = super.cached_context.getSelected();
	...
	public void close() {
		...
		if (this.returnNode != null) {
			super.cached_context.setSelected(returnNode);
	...
	public void render(Context context) {
		...
		// Get my width, and resize internal buttons so that they match the width of the
		// box
		...
		NVGPaint paint = NanoVG.nvgBoxGradient(vg, x + 2, y + 3, w - 2, h, 4, 12, Theme.current().getShadow().getNVG(),
				Color.TRANSPARENT.getNVG(), NVGPaint.create());
