...
import lwjgui.ClientSync;
import lwjgui.TaskManager;
import lwjgui.ThreadUtils;
...
public class Window extends Thread {
			...
			public void invoke(long window, double x, double y) {
				...
				if (selected.mousePressed) {
					...
					if (selected.mouseDraggedEvent != null) {
						EventHelper.fireEvent(selected.mouseDraggedEvent,
								new MouseEvent(x, y, GLFW.GLFW_MOUSE_BUTTON_LEFT));
					}
		...
		GLFW.glfwSetKeyCallback(context.getWindowHandle(), new GLFWKeyCallback() {
			...
			private void notifyKeyInput(Node root, int key, int scancode, int action, int mods, boolean isCtrlDown,
				...
				if (event.action == GLFW.GLFW_REPEAT) {
					if (root.keyRepeatEventInternal != null
							&& EventHelper.fireEvent(root.keyRepeatEventInternal, event)) {
						return;
					}
	...
	public void run() {
		taskManager.init();
		context.create(this);
		Font.SANS.loadIntoContext(context);
		Font.COURIER.loadIntoContext(context);
		Font.CONSOLAS.loadIntoContext(context);
		Font.ARIAL.loadIntoContext(context);
		Font.DINGBAT.loadIntoContext(context);
		scene.cached_context = context;
		created = true;
	...
	public void closeWindow() {
		running = false;
		ThreadUtils.sleep(Long.MAX_VALUE);
		Callbacks.glfwFreeCallbacks(context.getWindowHandle());
	...
	public void setScene(Scene scene) {
		this.scene = scene;
		this.scene.cached_context = context;
	 ...
	 * Adds the following files as icons of varying sizes for the window.
	 * 
	 * @param filetype  - only uses the Files that end with this file extension. Set
	 *                  to null to just use any file.
	 * @param iconFiles - the array of files to check/load
	 */
	...
	public void setResizible(boolean resizable) {
		GLFW.glfwSetWindowAttrib(getContext().getWindowHandle(), GLFW.GLFW_RESIZABLE,
