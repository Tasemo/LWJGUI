...
public class BlurPane extends StackPane {
	...
	private void resizeBuffer() {
		...
		if (this.cached_context.isModernOpenGL()) {
			nanoImage = NanoVGGL3.nvglCreateImageFromHandle(this.cached_context.getNVG(), buffer.getTexId(), oldSize.x,
					oldSize.y, NanoVG.NVG_IMAGE_FLIPY);
		} else {
			nanoImage = NanoVGGL2.nvglCreateImageFromHandle(this.cached_context.getNVG(), buffer.getTexId(), oldSize.x,
					oldSize.y, NanoVG.NVG_IMAGE_FLIPY);
		}
	}
	...
	private void blit(Context context) {
		...
		// Blit
		GL30.glBlitFramebuffer(sx1, sy1, sx2, sy2, 0, 0, destwid, desthei, GL11.GL_COLOR_BUFFER_BIT, GL11.GL_NEAREST);
	...
	class BlurBuffer extends OffscreenBuffer {
		...
		public BlurBuffer(int width, int height, OffscreenBuffer source) {
			...
			this.source = source;
			if (true) { // TODO: query core context
		...
		public void render(Context context, int x, int y, int w, int h) {
			...
			GL20.glUniform4f(GL20.glGetUniformLocation(quadShader.getProgram(), "uColor"),
					internalBackground.getRed() / 255f - 0.5f, internalBackground.getGreen() / 255f - 0.5f,
					internalBackground.getBlue() / 255f - 0.5f, internalBackground.getAlpha() / 255f);
			GL20.glUniform1f(GL20.glGetUniformLocation(quadShader.getProgram(), "uBlurSize"), blurRadius);
			GL20.glUniform2f(GL20.glGetUniformLocation(quadShader.getProgram(), "uTexelSize"), 1.0f / (float) w,
					1.0f / (float) h);
