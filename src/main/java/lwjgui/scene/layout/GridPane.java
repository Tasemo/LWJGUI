...
public class GridPane extends Pane {
	...
	public GridPane() {
		...
		this.setOnKeyPressed(event -> {
			...
			if (event.key == GLFW.GLFW_KEY_TAB) {
				Node selected = super.cached_context.getSelected();
				...
				if (e != null) {
					super.cached_context.setSelected(e);
				}
			}
		});
	}
	...
	/**
	 * Adds the given element at the x/y position in the GridPane. E.G. Add an
	 * element at 1, 1 (one grid space right, one grid space down).
	 * 
	...
	class NodeFiller extends StackPane {
		...
		public void render(Context context) {
			this.clip(context);
			// LWJGUIUtil.fillRect(context, getX(), getY(), getWidth(), getHeight(),
			// Color.AQUA);
		}
	}
